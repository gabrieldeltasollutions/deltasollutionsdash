...
interface QuoteItem {
  id?: number; // ID do banco de dados (se existir)
  tempId: string; // ID temporário para gerenciar a lista
  machineId: string;
...
const emptyItem: Omit<QuoteItem, 'id' | 'tempId'> = {
  machineId: "",
  partDescription: "",
  quantity: "1",
  selectedMaterialId: "",
...
export default function EditQuote() {
  ...
  const { data: quoteData } = trpc.quotes.getById.useQuery({ id: quoteId });
  ...
  useEffect(() => {
    if (quoteData && machines && materials) {
      const loadedItems: QuoteItem[] = quoteData.items.map(item => ({
        id: item.id,
        tempId: crypto.randomUUID(),
        machineId: item.machineId.toString(),
        partDescription: item.partDescription,
        quantity: item.quantity.toString(),
        selectedMaterialId: item.materialId?.toString() || "",
        partWidthMm: item.partWidthMm?.toString() || "",
        partLengthMm: item.partLengthMm?.toString() || "",
        rawMaterialCost: (item.rawMaterialCost / 100).toFixed(2),
        toolingCost: (item.toolingCost / 100).toFixed(2),
        thirdPartyServicesCost: (item.thirdPartyServicesCost / 100).toFixed(2),
        machineTimeHours: item.machineTimeHours.toString(),
        setupTimeHours: item.setupTimeHours.toString(),
      ...
      setFormData({
        ...
        profitMargin: quoteData.profitMargin.toString(),
        taxRate: quoteData.taxRate.toString(),
        notes: quoteData.notes || "",
        items: loadedItems,
      });
      setIsLoading(false);
    }
  }, [quoteData, machines, materials]);
  ...
  const addItem = () => {
    setFormData((prev) => ({
      ...prev,
      items: [...prev.items, { ...emptyItem, tempId: crypto.randomUUID() }],
    }));
  };
  ...
  const removeItem = (tempId: string) => {
    ...
    // Se o item tem ID do banco, adicionar à lista de deletados
    const itemToRemove = formData.items.find(item => item.tempId === tempId);
    if (itemToRemove?.id) {
  ...
  const updateItem = (tempId: string, updates: Partial<QuoteItem>) => {
    setFormData((prev) => ({
      ...prev,
      items: prev.items.map((item) => 
        item.tempId === tempId ? recalculateItem({ ...item, ...updates }) : item
      ),
    }));
  };

  // Atualizar material e calcular custo automaticamente
  const handleMaterialChange = (itemTempId: string, materialId: string) => {
    const item = formData.items.find(i => i.tempId === itemTempId);
    if (!item) return;

    const material = materials?.find(m => m.id === parseInt(materialId));
    
    updateItem(itemTempId, { selectedMaterialId: materialId });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data = {
      ...
      items: formData.items.map((item) => ({
        id: item.id, // Incluir ID se existir
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Editar Orçamento</h1>
          <p className="text-muted-foreground mt-2">
            Modifique os dados do orçamento #{quoteId}
          </p>
        </div>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateItem(item.tempId, { partDescription: e.target.value })}
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => updateItem(item.tempId, { machineId: value })}
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-3">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => handleMaterialChange(item.tempId, value)}
                    >
                      <SelectTrigger>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            updateItem(item.tempId, { partWidthMm: newWidth });
                            ...
                            if (item.selectedMaterialId && newWidth && item.partLengthMm) {
                              ...
                              if (material) {
                                ...
                                const cost = (partArea * material.costPerMm2) / 100000;
                                updateItem(item.tempId, { partWidthMm: newWidth, rawMaterialCost: cost.toFixed(2) });
                              }
                            }
                          }}
                          placeholder="100"
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            updateItem(item.tempId, { partLengthMm: newLength });
                            ...
                            if (item.selectedMaterialId && item.partWidthMm && newLength) {
                              ...
                              if (material) {
                                ...
                                const cost = (partArea * material.costPerMm2) / 100000;
                                updateItem(item.tempId, { partLengthMm: newLength, rawMaterialCost: cost.toFixed(2) });
                              }
                            }
                          }}
                          placeholder="200"
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateItem(item.tempId, { rawMaterialCost: e.target.value })}
                        placeholder="0.00"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateItem(item.tempId, { toolingCost: e.target.value })}
                        placeholder="0.00"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateItem(item.tempId, { thirdPartyServicesCost: e.target.value })}
                        placeholder="0.00"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateItem(item.tempId, { machineTimeHours: e.target.value })}
                        placeholder="120"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => updateItem(item.tempId, { setupTimeHours: e.target.value })}
                        placeholder="30"
                      />
                    </div>
                  </div>
